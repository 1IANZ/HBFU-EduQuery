...
pub fn parse_dekt_list(html: &str) -> ApiResult<Vec<DEKTList>> {
    ...
    let table = document
        ...
        .next()
        .ok_or_else(|| ApiError::Custom("未找到 id=dataList 的表格".to_string()))?;
...
pub fn parse_dekt_total(html: &str) -> ApiResult<Vec<DEKTTotal>> {
    let document = Html::parse_document(html);
    ...
    let table = document
        .select(&table_selector)
        .next()
        .ok_or_else(|| ApiError::Custom("未找到 class=Nsb_r_list 的表格".to_string()))?;
...
pub fn parse_dekt_detail(html: &str) -> ApiResult<DEKTDetail> {
    let document = Html::parse_document(html);
    ...
    let table = document
        .select(&table_selector)
        .next()
        .ok_or_else(|| ApiError::Custom("未找到 class=dataTable 的表格".to_string()))?;
...
pub fn parse_dekt(html: &str) -> ApiResult<DEKT> {
