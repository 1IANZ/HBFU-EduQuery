...
pub fn parse_elective(html: &str) -> ApiResult<ElectiveResponse> {
    let document = Html::parse_document(html);
    let table_selector = Selector::parse("table.Nsb_r_list.Nsb_table")
        .map_err(|_| ApiError::Custom("无法找到选课表格".to_string()))?;
    ...
    if tables.len() < 2 {
        return Err(ApiError::Custom("无法找到选课表格".to_string()));
    }
    ...
    for row in tables[0].select(&row_selector) {
        ...
        if cells.len() >= 6 {
            credits.push(CreditInfo {
                ...
                required: cells[1].text().collect::<String>().trim().parse().unwrap_or(0),
                limited: cells[2].text().collect::<String>().trim().parse().unwrap_or(0),
                elective: cells[3].text().collect::<String>().trim().parse().unwrap_or(0),
                public: cells[4].text().collect::<String>().trim().parse().unwrap_or(0),
                total: cells[5].text().collect::<String>().trim().parse().unwrap_or(0),
            });
        }
    }
    ...
    for row in tables[1].select(&row_selector) {
        ...
        if cells.len() >= 8 {
            courses.push(XqxkchInfo {
                ...
                hours: cells[3].text().collect::<String>().trim().parse().unwrap_or(0),
                credits: cells[4].text().collect::<String>().trim().parse().unwrap_or(0.0),
