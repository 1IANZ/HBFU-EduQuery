...
pub fn parse_student_info(html: &str) -> ApiResult<StudentInfo> {
    let document = Html::parse_document(html);
    let table_selector = Selector::parse("#xjkpTable").unwrap();
    let table = document.select(&table_selector).next().ok_or_else(|| ApiError::Custom("无法找到学籍信息表格".to_string()))?;
    ...
    Ok(StudentInfo {
        ...
        class: get_text(info_cells.get(3).cloned(), None).replace("班级：", ""),
        admission_date: get_text(admission_date_cells.get(1).cloned(), None).replace('\u{a0}', "").trim().to_string(),
        admission_number: get_text(id_number_cells.get(1).cloned(), None).replace('\u{a0}', "").trim().to_string(),
        id_number: get_text(id_number_cells.get(3).cloned(), None).replace('\u{a0}', "").trim().to_string(),
    })
}