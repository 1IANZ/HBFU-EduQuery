...
pub fn parse_plan(html: &str) -> ApiResult<ExecutionPlanResponse> {
    let document = Html::parse_document(html);

    let selector_str = "table#dataList.Nsb_r_list.Nsb_table";
    let selector = Selector::parse(selector_str)
        .map_err(|_| ApiError::Custom("选择器解析失败".to_string()))?;

    let table = document
        .select(&selector)
        .next()
        .ok_or_else(|| ApiError::Custom("未找到符合条件的表格元素".to_string()))?;

    let tr_selector =
        Selector::parse("tr").map_err(|_| ApiError::Custom("tr选择器解析失败".to_string()))?;
    let td_selector =
        Selector::parse("td").map_err(|_| ApiError::Custom("td选择器解析失败".to_string()))?;
